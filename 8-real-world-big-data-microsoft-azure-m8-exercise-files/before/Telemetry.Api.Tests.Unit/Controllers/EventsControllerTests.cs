    ...
    public class EventsControllerTests
    {
        ...
        public async Task Post_SenderException_ShouldReturn500()
        {
            var events = new
            {
                events = new[] {
                    new {
                        deviceId = "Post_SenderException_ShouldReturn500",
                        eventName = "device.activated",
                        timestamp = 1415807587310
                    }
                }
            ...
            var request = new HttpRequestMessage();
            request.Headers.Add("x-device-id", "Post_SenderException_ShouldReturn500");
            request.Content = new StringContent(JsonConvert.SerializeObject(events));
            ...
            _senderMock.Verify(x => x.SendEventsAsync(It.Is<JArray>(e => e.Count == 1), "Post_SenderException_ShouldReturn500"), Times.Once);
        }


        [TestMethod]
        public async Task Post_NoEvents_ShouldReturn200()
        {
            var request = new HttpRequestMessage();
        ...
        [TestMethod]
        public async Task Post_SendDisabled_ShouldReturn200()
        {
