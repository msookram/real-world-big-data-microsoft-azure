...
$SenderName = "api-sender"
[Microsoft.ServiceBus.Messaging.SharedAccessAuthorizationRule] $SenderRule = $null
if ($EventHubDescription.Authorization.TryGetSharedAccessAuthorizationRule($SenderName, [ref] $SenderRule) -eq $False)
{
    # add the sender access rights    
    $SenderKey = [Microsoft.ServiceBus.Messaging.SharedAccessAuthorizationRule]::GenerateRandomKey()
    [Microsoft.ServiceBus.Messaging.AccessRights[]]$SenderRights = New-Object -TypeName "System.Collections.Generic.List[Microsoft.ServiceBus.Messaging.AccessRights]"
    $SenderRights += [Microsoft.ServiceBus.Messaging.AccessRights]::Send
    [Microsoft.ServiceBus.Messaging.SharedAccessAuthorizationRule] $SenderRule = New-Object -TypeName Microsoft.ServiceBus.Messaging.SharedAccessAuthorizationRule -ArgumentList $SenderName, $SenderKey, $SenderRights
    $SenderRule.SecondaryKey = [Microsoft.ServiceBus.Messaging.SharedAccessAuthorizationRule]::GenerateRandomKey()
    Write-Output "Adding [$SenderRights] for [$SenderName]."     
    $EventHubDescription.Authorization.Add($SenderRule)
...
Write-Output "Sender connection string: "
Write-Output "Endpoint=sb://$Namespace.servicebus.windows.net/;SharedAccessKeyName=$($SenderRule.KeyName);SharedAccessKey=$($SenderRule.PrimaryKey)"
